...
const getCat = async (req, res, next) => {
  const { jenisKelamin, umur, moreThanUmur, lessThanUmur } = req.query;
  try {
...
const insertCat = async (req, res, next) => {
  const { ras, nama, umur, jenisKelamin, idPelanggan } = req.body;
  try {
    console.log(idPelanggan);
    if (typeof umur != "number") {
      return next(new ErrorHandler("Age data must be a number ", 404));
    }
    await Cat.create({ idPelanggan, nama, ras, umur, jenisKelamin });
...
const updateCat = async (req, res, next) => {
  const { nama, ras, umur, jenisKelamin } = req.body;
  const { id } = req.params;
  try {
    if (typeof umur != "number") {
      return next(new ErrorHandler("Age data must be a number ", 404));
    ...
    await Cat.update(id, nama, ras, umur, jenisKelamin);
